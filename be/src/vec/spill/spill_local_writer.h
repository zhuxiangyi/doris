...
// 本地磁盘 SpillWriter
class SpillLocalWriter : public SpillWriterBase {
public:
    SpillLocalWriter(std::shared_ptr<ResourceContext> resource_context, RuntimeProfile* profile,
                     int64_t id, size_t batch_size, SpillDataDir* data_dir,
                     const std::string& dir)
            : data_dir_(data_dir),
              stream_id_(id),
              batch_size_(batch_size),
              _resource_ctx(std::move(resource_context)) {
        // Directory path format: storage_root/spill/query_id/operator_name-node_id-task_id-stream_id/0
        file_path_ = dir + "/0";
    ...
    const std::string& get_file_path() const override { return file_path_; }
...
using SpillLocalWriterUPtr = std::unique_ptr<SpillLocalWriter>;
} // namespace vectorized
} // namespace doris
