...
#include "vec/spill/spill_local_writer.h"
...
Status SpillLocalWriter::write(RuntimeState* state, const Block& block, size_t& written_bytes) {
    written_bytes = 0;
    DCHECK(file_writer_);
...
Status SpillLocalWriter::_write_internal(const Block& block, size_t& written_bytes) {
...
void SpillLocalWriter::set_counters(RuntimeProfile* operator_profile) {
